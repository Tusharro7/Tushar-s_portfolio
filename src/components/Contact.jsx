...
//     setUserEmail('');
//     setUserText('');
//     console.log(userName , userEmail , userText);
//     alert("Message sent!");
...
import React, { useContext, useState, useEffect, useRef } from "react"; // 1. useRef import kiya
import { ThemeContext } from "../components/theme/ThemeContext";
import ScrollFadeIn from "../components/scrollfadein/ScrollFadeIn";
import { FaGithub, FaLinkedin, FaEnvelope, FaPhone } from "react-icons/fa";
import { motion } from "framer-motion";
...
const Contact = () => {
  ...
  // 3. Form ka reference banaya taaki EmailJS isse read kar sake
  const form = useRef(); 
  ...
  const handleSubmit = (e) => {
    ...
    // Yahan apni IDs replace karein
    const serviceID = "service_tes24qv"; 
    const templateID = "template_qh7yn25"; 
    const publicKey = "V60JMzdH7Qw_jFGXv"; 
    ...
    <section id="contact" className="relative z-10 px-6 md:px-20 py-16">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <ScrollFadeIn className="w-full md:w-1/2">
          ...
          {/* 5. Form tag me 'ref={form}' add kiya */}
          <form
            ref={form} 
            onSubmit={handleSubmit}
            className={`flex flex-col gap-4 p-6 rounded-xl shadow-lg bg-white/10 backdrop-blur-md ${textColor}`}
          >
            <input
              type="text"
              name="user_name" // 6. Name attribute zaroori hai
              placeholder="Your Name"
              value={userName}
              onChange={userNameHandeler}
              className={`px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-white ${textColor} border ${darkMode ? "border-white" : "border-black"}`}
            ...
            <input
              type="email"
              name="user_email" // 6. Name attribute zaroori hai
              placeholder="Your Email"
              value={userEmail}
              onChange={userEmailHandeler}
              className={`px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-white ${textColor} border ${darkMode ? "border-white" : "border-black"}`}
            ...
            />
            <textarea
              name="message" // 6. Name attribute zaroori hai
              placeholder="Your Message"
              value={userText}
              onChange={userTextHandeler}
              rows="5"
